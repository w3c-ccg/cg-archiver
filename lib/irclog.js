...
function parseTranscript({transcript}) {
  ...
  // Check first 10 lines for an "attendees" line and process the next line
  for(let i = 0; i < Math.min(10, lines.length); i++) {
    if(/^attendees$/i.test(lines[i].trim())) {
      const attendeeLine = lines[i + 1]?.trim();
      if(attendeeLine) {
        for(const attendee of attendeeLine.split(',')) {
          const speaker = attendee.trim();
          if(speaker) {
