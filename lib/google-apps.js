...
export async function downloadVideo({driveLocation, fileStream}) {
  // wait for OAuth2 authorization to Google Meet
  const auth = await authorize();
...
export async function downloadTranscript({mediaType, docLocation, fileStream}) {
  // wait for OAuth2 authorization to Google Meet
  const auth = await authorize();
  const driveClient = google.drive({version: 'v3', auth});
...
export async function uploadToArchive({fileStream, filename, archiveConfig}) {
  const sftp = new SftpClient('sftp-archive-client');
  const sftpConfig = {
    host: archiveConfig.host,
    username: archiveConfig.username,
    privateKey: archiveConfig.privateKey
  };
  const destinationFilename = path.join(archiveConfig.directory, filename);
