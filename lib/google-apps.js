...
export async function generateHtml({config, meeting, date, transcript}) {
  let summary = `${meeting.name} transcript for ${date}`;
  const genAI = new GoogleGenerativeAI(config.geminiApiKey);
...
two inputs, a transcript in text format and an HTML template. You will
produce a valid HTML document of the meeting as output. Perform the following
steps:

1. Process every line of the transcript, it is vital that no line is removed in the output.
...
5. Organize the transcript into less than 9 topics, summarizing each topic name using less than 9 words.
6. For each topic, create a section in the HTML using the following template: "<section><h3 id="FRAGMENT">TOPIC_NAME</h3></section>" where TOPIC_NAME is the name of the topic and FRAGMENT is a unique topic fragment identifier that will be used later when generating links to each section in the AGENDA value.
6.1 For every line of the transcript, in order, output the transcript line using the following HTML snippet "<p id="FRAGMENT" class="phone s01"><cite>SPEAKER_NAME:</cite> UTTERANCE</p>" as a template. The FRAGMENT value is replaced with a unique fragment identifier. The SPEAKER_NAME is replaced with the name of the person speaking, which is the value before the first colon in the transcript. The UTTERANCE value is replaced with the rest of the transcript line after the colon. Fix any syntax and grammar in the statement without changing the meaning of the statement. You must ensure that every line of the transcript appears in the output.
7. Replace the AGENDA value in the HTML template with the list of topics where each topic uses the following HTML snippet "<li><a href="#FRAGMENT">TOPIC_NAME</a></li>" where FRAGMENT is the fragment identifier for the respective section and TOPIC_NAME is the name of the topic.
